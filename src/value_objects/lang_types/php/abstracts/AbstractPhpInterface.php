 ...
 * 抽象言語型：PHP：interface
 */
abstract readonly class AbstractPhpInterface implements ValueObjectInterface
{
    /**
     * constructor
     *
    ...
    {
        if (!\interface_exists($value, $autoload)) {
            throw new \ValueError(\sprintf('存在しないまたはアクセスできないインターフェース（"%s"）を指定されました。', $value));
        }
    }
}
