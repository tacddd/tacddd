...
use tacddd\value_objects\lang_types\php\PhpList;
 ...
 * @internal
 */
#[CoversClass(AbstractPhpList::class)]
#[CoversClass(PhpList::class)]
class PhpListTest extends AbstractValueObjectTestCase
{
        ...
        return new class() extends AbstractValueObjectClassSpec {
            ...
            public function getExpectedUsingTraits(): array
            {
                return [
                    ...
                    PhpListFactoryMethodTrait::class,
                ];
            }
        };
    }

    public static function notListDataProvider(): iterable
    {
        foreach (TypeDataSet::typeDataSetWithInclusion([
            'array',
        ]) as $type_data_set) {
    ...
    #[Test]
    #[DataProvider('notListDataProvider')]
    #[TestDox('notList [#$_dataName] value: $value')]
    public function notList(mixed $value): void
    {
        $this->expectException(\ValueError::class);
        $this->expectExceptionCode(0);
        $this->expectExceptionMessage('リストではない配列を指定されました。');
