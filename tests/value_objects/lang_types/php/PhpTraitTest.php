...
use tacddd\value_objects\lang_types\php\abstracts\AbstractPhpTrait;
use tacddd\value_objects\lang_types\php\PhpTrait;
use tacddd\value_objects\lang_types\php\traits\factory_methods\PhpTraitFactoryMethodTrait;

/**
 * 言語型：PHP：trait
 * @internal
 */
class PhpTraitTest extends AbstractValueObjectTestCase
{
    ...
    {
        foreach (TypeDataSet::typeDataSetWithInclusion(['trait']) as $type_data_set) {
    ...
    public static function getClassSpec(): AbstractValueObjectClassSpec
    {
        return new class() extends AbstractValueObjectClassSpec {
            ...
            {
                return [
                    PhpTraitFactoryMethodTrait::class,
                ];
            }
        };
    }

    public static function notExistsTraitPathDataProvider(): iterable
    {
        foreach (TypeDataSet::typeDataSetWithInclusion([
            'empty_string',
            'multibyte_string',
        ...
        yield ['\tacddd\NotExistsTrait', '\tacddd\NotExistsTrait'];
    }

    #[Test]
    #[DataProvider('notExistsTraitPathDataProvider')]
    #[TestDox('notExistsTraitPath [#$_dataName] value: $value, type: $type')]
    public function notExistsTraitPath(mixed $value, string $type): void
    {
        $this->expectException(\ValueError::class);
        $this->expectExceptionCode(0);
        $this->expectExceptionMessage(\sprintf(
            '存在しないまたはアクセスできないトレイト（"%s"）を指定されました。',
            $type,
        ));
