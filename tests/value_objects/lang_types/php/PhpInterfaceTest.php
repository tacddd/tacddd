...
use tacddd\value_objects\lang_types\php\abstracts\AbstractPhpInterface;
use tacddd\value_objects\lang_types\php\PhpInterface;
use tacddd\value_objects\lang_types\php\traits\factory_methods\PhpInterfaceFactoryMethodTrait;

/**
 * 言語型：PHP：trait
 * @internal
 */
class PhpInterfaceTest extends AbstractValueObjectTestCase
{
    ...
    {
        foreach (TypeDataSet::typeDataSetWithInclusion(['interface']) as $type_data_set) {
    ...
    public static function getClassSpec(): AbstractValueObjectClassSpec
    {
        return new class() extends AbstractValueObjectClassSpec {
            ...
            {
                return [
                    PhpInterfaceFactoryMethodTrait::class,
                ];
            }
        };
    }

    public static function notExistsInterfacePathDataProvider(): iterable
    {
        foreach (TypeDataSet::typeDataSetWithInclusion([
            'empty_string',
            'multibyte_string',
        ...
        yield ['\tacddd\NotExistsInterface', '\tacddd\NotExistsInterface'];
    }

    #[Test]
    #[DataProvider('notExistsInterfacePathDataProvider')]
    #[TestDox('notExistsInterfacePath [#$_dataName] value: $value, type: $type')]
    public function notExistsInterfacePath(mixed $value, string $type): void
    {
        $this->expectException(\ValueError::class);
        $this->expectExceptionCode(0);
        $this->expectExceptionMessage(\sprintf(
            '存在しないまたはアクセスできないインターフェース（"%s"）を指定されました。',
            $type,
        ));
