...
use tacddd\value_objects\lang_types\php\PhpEnum;
 ...
 * @internal
 */
class PhpEnumTest extends AbstractValueObjectTestCase
{
    /**
     * アジャスタ用データプロバイダ
     *
    ...
    {
        foreach (TypeDataSet::typeDataSetWithInclusion(['enum']) as $type_data_set) {
    ...
    public static function getClassSpec(): AbstractValueObjectClassSpec
    {
        return new class() extends AbstractValueObjectClassSpec {
            ...
            {
                return [
                    PhpEnumFactoryMethodTrait::class,
                ];
            }
        };
    }

    public static function notExistsEnumPathDataProvider(): iterable
    {
        foreach (TypeDataSet::typeDataSetWithInclusion([
            'std_class_object',
            'object',
    ...
    #[Test]
    #[DataProvider('notExistsEnumPathDataProvider')]
    #[TestDox('notExistsEnumPath [#$_dataName] value: $value, type: $type')]
    public function notExistsEnumPath(mixed $value, string $type): void
    {
        $this->expectException(\ValueError::class);
        $this->expectExceptionCode(0);
        $this->expectExceptionMessage(\sprintf(
            '存在しないまたはアクセスできない列挙型（"%s"）を指定されました。',
            $type,
        ));
